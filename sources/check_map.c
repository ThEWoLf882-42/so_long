...
/*   Updated: 2023/02/18 20:13:28 by agimi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	up_do_ch(t_all *all, t_map *m)
{
	...
	while (m->s[++i] && i < all->mpx)
		if (m->s[i] != '1')
			get_out_s(all, "What kind of walls this is1\n");
	m = ft_lstlast_m(m);
	i = -1;
	...
	while (m->s[++i] && i < all->mpx)
		if (m->s[i] != '1')
			get_out_s(all, "What kind of walls this is2\n");
}
...
void	corner_ch(t_all *all, t_map *m)
{
	...
	while (++i < all->mpy - 1)
	{
		...
		if (m->s[0] != '1' || m->s[all->mpx - 1] != '1')
			get_out_s(all, "What kind of walls this is3\n");
		m = m->next;
	}
}
