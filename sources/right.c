...
/*   Updated: 2023/02/14 12:37:41 by agimi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rloop(t_all *all)
{
	...
	if (i >= 0 && i <= 8)
		all->my.player = mlx_texture_to_image(all->my.mlx, all->mv.right.r2);
	if (i >= 8 && i <= 16)
		all->my.player = mlx_texture_to_image(all->my.mlx, all->mv.right.r1);
	if (i >= 16 && i <= 24)
		all->my.player = mlx_texture_to_image(all->my.mlx, all->mv.right.r0);
	if (i >= 24 && i <= 32)
		all->my.player = mlx_texture_to_image(all->my.mlx, all->mv.right.r1);
	if (i == 32)
		i = 0;
